...
int	main(int argc, char **argv)
{
	...
	for(i=0;i<NumberOfFirstLayerFFNNs;i++){
		...
		if( !PadMe(NumberOfFFNN, i+1, buffer) ){
			fprintf(stderr, "%s: PadMe failed\a\n", argv[0]);
			free(buffer); free(InputsToFirstLayerFFNNs);
			exit(1);
		}
		printf("%d]),(Y[1]),FFNN_%s,(%d,1),1,%d;\n", these_inputs, buffer, InputsToFirstLayerFFNNs[i],NumberOfFirstLayerFFNNs);
		these_inputs++;
	}
